...
plot(y, main = "Plots of our ten variables", cex.lab = 0.7, col = "blue")
plot(HOURS, HHSAVINGS, ylab = "", type = "l", main = "Plots of our ten variables", cex.lab = 0.7, col = "blue")
...
$= [\kappa_A^Kdet(\underline{V})]^\frac{N}{2}(\kappa_A)^\frac{-\underline{\nu}_A+2}{2}exp[-\frac{1}{2}\frac{\underline{s}_A + tr[\Sigma^{-1}(A-\underline{A})^{'}(\kappa_A \underline{V})^{-1}(A-\underline{A}))]}{\kappa_A}$
...
With : $\bar{s}_A =  \underline{s}_A + tr[\Sigma^{-1}(A-\underline{A})^{'}(\kappa_A \underline{V})^{-1}(A-\underline{A}))]$
...
$= det(\kappa_\Sigma \underline{S})^\frac{-\underline{\nu}}{2}exp\{-\frac{1}{2}tr[\Sigma^{-1}(\kappa_\Sigma\underline{S})]exp\{-\frac{\kappa_\Sigma}{\beta_\Sigma}(\kappa_\Sigma)^{\alpha_\Sigma-1}\}$

$= \kappa_\Sigma^\frac{-N\underline{\nu}}{2} det(\underline{S})^\frac{\underline{\nu}}{2}exp(-\frac{1}{2}[\Sigma^{-1}(\kappa_\Sigma \underline{S})]+\frac{\kappa_\Sigma}{\beta_\Sigma})$

$=\kappa_\Sigma^{\frac{-N\underline{\nu}}{2}+\alpha -1}exp[-\frac{1}{2}tr(\Sigma^{-1} \underline{S})+\frac{1}{\beta_\Sigma})\kappa_\Sigma]$

Hence : Hence $\kappa_\Sigma \sim IG(\bar{\alpha}_\Sigma,\bar{\beta}_\Sigma)$
...
Allowing us to obtain a (5 x 10 x S) array of forecasts $y_{t+1}^{(s)},...,y_{t+4}^{(s)}^{S=1000}_s=1$ for each period up until the desired forecast horizon.
...
xtable(rbind(format(t(A_LIST[2:11,]),digits=1, scientific=FALSE)[1,],
             format(t(A_SD[2:11,]),digits=1, scientific=FALSE)[1,])

out                        =rbind(A_SAMP_MEAN[2:11,],A_SAMP_SD[2:11,])
colnames(out)              =c("$A_1$")

A1_TABLE= kable(round(out,5), caption="A_1 Posterior Estimates of AR(4) Model")
...
For both the hyper-parameters $\kappa_A$ and $\kappa_\Sigma$ the distributions are centered around 0.022 and $1.5 x 10^7$ respectively. The large estimates of the shrinkage parameters $\kappa_Sigma$ is caused by the consistent deviation of the distributional mass of the posterior from its mean. Both parameters have right-skewed distributional shapes with $\kappa_Sigma$ exhibiting a slightly higher modal concentration and defined right-tail. 
